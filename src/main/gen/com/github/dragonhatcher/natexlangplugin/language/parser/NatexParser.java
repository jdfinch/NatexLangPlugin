...
public class NatexParser implements PsiParser, LightPsiParser {
  ...
  /* ********************************************************** */
  // KEYWORD | state_name | term (","? " "? term)*
  static boolean natexFile(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "natexFile")) return false;
    boolean r;
    Marker m = enter_section_(b);
    r = consumeToken(b, KEYWORD);
    if (!r) r = state_name(b, l + 1);
    if (!r) r = natexFile_2(b, l + 1);
    exit_section_(b, m, null, r);
    return r;
  }

  // term (","? " "? term)*
  private static boolean natexFile_2(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "natexFile_2")) return false;
    boolean r;
    Marker m = enter_section_(b);
    r = term(b, l + 1);
    r = r && natexFile_2_1(b, l + 1);
    exit_section_(b, m, null, r);
    return r;
  }

  // (","? " "? term)*
  private static boolean natexFile_2_1(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "natexFile_2_1")) return false;
    while (true) {
      int c = current_position_(b);
      if (!natexFile_2_1_0(b, l + 1)) break;
      if (!empty_element_parsed_guard_(b, "natexFile_2_1", c)) break;
    }
    return true;
  }

  // ","? " "? term
  private static boolean natexFile_2_1_0(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "natexFile_2_1_0")) return false;
    boolean r;
    Marker m = enter_section_(b);
    r = natexFile_2_1_0_0(b, l + 1);
    r = r && natexFile_2_1_0_1(b, l + 1);
    r = r && term(b, l + 1);
    exit_section_(b, m, null, r);
    return r;
  }
  ...
  // ","?
  private static boolean natexFile_2_1_0_0(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "natexFile_2_1_0_0")) return false;
    consumeToken(b, ",");
    return true;
  }

  // " "?
  private static boolean natexFile_2_1_0_1(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "natexFile_2_1_0_1")) return false;
    consumeToken(b, " ");
    return true;
  }
